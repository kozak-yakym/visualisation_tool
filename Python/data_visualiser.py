...
log_path = os.path.abspath(os.path.join(__file__, DATASET_PATH, 'log.txt'))
...
plot1 = figure(title="Графіки нагріву котла в Каневі",
...
# plot1.vbar(width=0.9, legend_label="Loads", color=next(line_colors), source=source )

plot1.circle(load_timedate_list, load_amount_list,
             legend_label="Loads", color=next(line_colors))
...
'''
# FIXME: Remember why I needed to debug this!!!
...
l1 = p.line(x, 4 * np.cos(x),
            color=Viridis3[1], legend_label="Line 1", **props)
l2 = p.line(x, np.tan(x), color=Viridis3[2], legend_label="Line 2", **props)

checkbox = CheckboxGroup(labels=["Line 0", "Line 1", "Line 2"],
                         active=[0, 1, 2], width=100)

checkbox.js_on_change("active", CustomJS(args=dict(l0=l0, l1=l1, l2=l2, checkbox=checkbox),
                              code="""
...
s1 = figure(min_width=250, height=250, title=None)
s1.circle(x, y0, size=10, color="navy", alpha=0.5)

s2 = figure(min_width=250, height=250, title=None)
s2.triangle(x, y1, size=10, color="firebrick", alpha=0.5)

s3 = figure(min_width=250, height=250, title=None)
...
plot = figure(min_width=400, height=400)
...
checkbox2 = CheckboxGroup(labels=["Cosinus", "Sinus"], active=[0, 1])

callback = CustomJS(args=dict(line0=a, line1=b), code="""
...
checkbox2.js_on_change('active', callback)

# show(row(checkbox2, col))
